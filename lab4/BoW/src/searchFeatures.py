...
for i, image_path in enumerate(image_paths):
    ...
    print ("提取了图片 %s 的ORB特征, %d of %d images" %(training_names[i], i, len(image_paths)))
...
for des in des_list: #des_list内含38个tuple
    for each_fea in des[1]: #des[0] = image_path，des[1] = 500*32大小的二维数组
        each_fea_list.append((des[0], each_fea)) #得到32维descriptor
...
#codebook有1000个元素，每个元素是一个32维的聚类中心（ORB是32维，SIFT是128维）
...
#            mindist = dist
	...
	left_up = (int(tmp_fea_structure[0].pt[0] - r / 2.0), int(tmp_fea_structure[0].pt[1] - r / 2.0))
	right_down = (int(tmp_fea_structure[0].pt[0] + r / 2.0), int(tmp_fea_structure[0].pt[1] + r / 2.0))
	...
	print (tmp_fea_structure[0].size)
        cv2.circle(img1, (int(tmp_fea_structure[0].pt[0]), int(tmp_fea_structure[0].pt[1])), int(tmp_fea_structure[0].size) ,(255,255,0), 3)
